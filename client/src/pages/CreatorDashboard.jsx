...
function CreatorDashboard() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => (await api.get("/users/dashboard")).data,
  });
  const mine = (data?.prompts || []).map(promptView);
  const stats = data?.stats || {};
  const earningsBars = Array.from({ length: 7 }).map((_, i) => {
    const prompt = mine[i % (mine.length || 1)];
    return prompt ? Math.max(0, Math.round(prompt.salesCount * prompt.price * 0.75)) : 0;
  });
  const maxBar = Math.max(...earningsBars, 1);
