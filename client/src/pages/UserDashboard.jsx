  ...
  return (
    <section className="mx-auto max-w-[1400px] px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-bg-card p-6"><h2 className="font-heading text-xl font-semibold">Recent Activity</h2><div className="mt-5 space-y-4">{purchases.slice(0, 5).map((purchase, index) => { const prompt = purchase.prompt; return <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} key={purchase._id || index} className="flex gap-4 border-l border-indigo-500/30 pl-4"><ShoppingBag className="mt-1 text-indigo-400" size={18} /><div><p className="font-semibold">{prompt?.title || "Unknown Prompt"}</p><p className="text-sm text-text-muted">{new Date(purchase.createdAt).toLocaleDateString()} · {purchase.tokensSpent} tokens spent</p></div></motion.div>; })}</div></div>
