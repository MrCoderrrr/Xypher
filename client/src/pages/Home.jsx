...
const fetchStats = async () => {
  try {
    ...
    return { prompts: 0, creators: 0, generations: 0, avgRating: 0, creatorEarnings: 0, weeklyGenerations: [] };
  }
};

const categoryIcons = {
  Writing: PenTool, Coding: Code2, Marketing: Megaphone, Image, Video,
  Business: TrendingUp, Education: GraduationCap, SEO: Search, "Social Media": Play
};

const heroFallbackCards = [
  { id: "hero-1", title: "Prompt Marketplace", description: "Data updates as prompts are approved and purchased.", previewImage: "", rating: "0.0", price: 0 },
  { id: "hero-2", title: "Creator Portal", description: "Upload and monetize prompt systems.", previewImage: "", rating: "0.0", price: 0 },
  { id: "hero-3", title: "Buyer Portal", description: "Purchase prompts and run generations.", previewImage: "", rating: "0.0", price: 0 },
  { id: "hero-4", title: "Admin Portal", description: "Review prompts, creators, and payouts.", previewImage: "", rating: "0.0", price: 0 },
];

const DEFAULT_CATEGORIES = ["Writing","Coding","Marketing","Image","Video","Business","Education","SEO","Social Media"];
      ...
      {displayCards.map((prompt, i) => {
        ...
        return (
          <motion.div
            ...
            initial={false}
            animate={{ x: pos.x, y: pos.y, scale: pos.scale, zIndex: pos.zIndex, opacity: pos.opacity }}
            transition={{ duration: 0.6, ease: "easeInOut" }}
          >
            <div
              className="animate-float rounded-2xl border border-border bg-bg-card p-4 shadow-[0_30px_80px_rgba(99,102,241,0.25)]"
            ...
            >
              {prompt.previewImage ? (
                ...
                <div className="aspect-video w-full rounded-xl border border-border bg-bg-elevated/70" />
              )}
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-amber-300">★ {prompt.rating || "0.0"}</span>
                <span className="text-cyan">⚡ {prompt.price || 0}</span>
              </div>
            </div>
          </motion.div>
        );
...
function Home() {
  ...
  const { data: categoryData } = useQuery({
    ...
    queryFn: async () => {
      try {
        return (await api.get("/prompts/categories")).data;
      } catch {
  ...
  const { data: testimonialsData } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const promptIds = (featured || []).slice(0, 3).map((p) => p._id || p.id);
        const reviews = await Promise.all(
          promptIds.map(async (id) => {
            try {
              return (await api.get(`/prompts/${id}/reviews`)).data.reviews || [];
            } catch {
              return [];
            }
          })
  ...
  const categories = categoryData?.categories || DEFAULT_CATEGORIES;
  ...
  const weeklyValues = displayStats.weeklyGenerations?.length
    ? displayStats.weeklyGenerations
    : [0, 0, 0, 0, 0, 0, 0];
  const maxWeeklyValue = Math.max(...weeklyValues, 1);
  const dynamicBarHeights = weeklyValues.map((v) => Math.max(12, Math.round((v / maxWeeklyValue) * 100)));
    ...
    <>
      ...
      <section className="hero-radial dot-grid relative overflow-hidden pt-0 lg:pt-4">
        <div className="mx-auto grid max-w-[1400px] items-start gap-12 px-4 pb-20 pt-8 sm:px-6 lg:px-8 lg:grid-cols-[1fr_0.9fr]">
          <motion.div variants={staggerContainer} initial="hidden" animate="visible">
            <motion.p variants={fadeUp} className="mb-5 inline-flex rounded-full border border-indigo-500/40 bg-indigo-500/10 px-4 py-2 text-sm font-semibold text-indigo-200 animate-glow">
            ...
            <motion.p variants={fadeUp} className="mt-6 max-w-2xl text-xl leading-relaxed text-text-muted">
              Discover battle-tested prompts from expert creators. Buy once, generate forever. Save hours every week.
            </motion.p>
            ...
            <motion.p variants={fadeUp} className="mt-5 text-sm text-text-muted">
              ⚡ {(displayStats.prompts || 0).toLocaleString()} prompts • {(displayStats.creators || 0).toLocaleString()} creators • {(displayStats.generations || 0).toLocaleString()} generations
            </motion.p>
          </motion.div>
      ...
      <Reveal className="border-y border-white/5 py-8">
        <div className="mx-auto grid max-w-[1400px] grid-cols-2 divide-x divide-white/5 px-4 sm:px-6 lg:px-8 md:grid-cols-4">
          {[
            ["Prompts", displayStats.prompts],
            ["Creators", displayStats.creators],
            ["Generations", displayStats.generations],
            ["Avg Rating", Math.round((displayStats.avgRating || 0) * 10)],
          ].map(([label, value]) => (
      ...
      <Reveal className="mx-auto max-w-[1400px] px-4 py-20 sm:px-6 lg:px-8">
        ...
        <div className="relative mt-14 grid gap-8 md:grid-cols-3">
          ...
          {[
            [Search, "Browse", "Explore prompts by category, creator, and outcome."],
            [Zap, "Buy", "Unlock reusable systems with tokens."],
            [Sparkles, "Generate", "Run prompts and get better outputs instantly."],
          ].map(([Icon, title, body], index) => (
            <motion.div key={title} variants={fadeUp} className="relative text-center">
              <div className="mx-auto flex h-[72px] w-[72px] items-center justify-center rounded-full border border-indigo-500/50 bg-bg-card p-1">
                <span className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-indigo-600 to-cyan-500 font-heading font-bold">
                  {index + 1}
                </span>
              </div>
      ...
      <Reveal className="bg-bg-elevated/40 py-20" variants={staggerContainer}>
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-end justify-between">
            <h2 className="font-heading text-4xl font-black">Featured Prompts</h2>
            <Link to="/explore" className="font-semibold text-cyan">View All →</Link>
          </div>
          {isLoading ? (
            ...
            <motion.div variants={staggerContainer} className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {(featured || []).slice(0, 6).map((prompt) => (
                <motion.div key={prompt._id || prompt.id} variants={scaleIn}>
                  <PromptCard {...prompt} id={prompt._id || prompt.id} />
                </motion.div>
              ))}
            </motion.div>
          ) : (
            <p className="text-center text-text-muted py-12">No prompts yet. Be the first to upload one!</p>
          )}
        </div>
      </Reveal>
          ...
          {categories.map((category) => {
            ...
            return (
              <Link
                ...
                className="flex h-20 w-[120px] shrink-0 snap-start flex-col items-center justify-center gap-2 rounded-xl border border-border bg-bg-card text-sm font-semibold text-text-muted hover:scale-105 hover:border-indigo-500 hover:text-cyan transition-all duration-200"
              >
      ...
      <section className="mx-auto grid max-w-[1400px] gap-10 px-4 py-20 sm:px-6 lg:px-8 lg:grid-cols-2">
        <Reveal>
          ...
          <p className="mt-4 text-xl leading-relaxed text-text-muted">
            Package your expertise once and sell it to a global audience of builders, marketers, founders, and AI operators.
          </p>
          <div className="mt-6 space-y-4">
            {["Keep 75% of every sale", "Global audience of AI builders", "Simple upload and review process", "Instant payout requests"].map((item) => (
              <p key={item} className="flex items-center gap-3 text-text-muted">
                <CheckCircle2 className="text-success shrink-0" /> {item}
              </p>
            ))}
          </div>
          <Link to="/upload" className="mt-8 inline-flex">
        ...
        <Reveal variants={slideInLeft}>
          <div className="rounded-2xl border border-border bg-bg-elevated p-6 shadow-card">
            ...
            <p className="mt-2 font-heading text-5xl font-bold">
              ₹{Math.round(displayStats.creatorEarnings || 0).toLocaleString()}
            </p>
            <div className="mt-8 flex h-40 items-end gap-3">
              {dynamicBarHeights.map((height, i) => (
                <div key={i} className="flex-1 rounded-t-lg bg-gradient-to-t from-indigo-600 to-cyan-400" style={{ height: `${height}%` }} />
              ))}
            </div>
            <p className="mt-5 text-sm font-semibold text-success">7-day generation trend</p>
          </div>
        </Reveal>
      </section>
        ...
        {testimonials.length ? (
          <motion.div variants={staggerContainer} className="mt-10 grid gap-6 md:grid-cols-3">
            {testimonials.map((item, i) => (
              <motion.div key={i} variants={scaleIn} className="rounded-2xl border border-border bg-bg-card p-6 hover:border-indigo-500 transition-colors">
                <div className="flex text-amber-300">
                  {Array.from({ length: item.rating }).map((_, j) => <Star key={j} size={16} fill="currentColor" />)}
                </div>
                <p className="mt-4 text-text-muted">"{item.text}"</p>
                <div className="mt-5 flex items-center gap-3">
                  <Avatar initials={item.avatarInitials} name={item.name} color={item.avatarColor} />
                  <div>
                    <p className="font-semibold">{item.name}</p>
                    <p className="text-sm text-text-muted">{item.role}, {item.company}</p>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <p className="mt-10 text-center text-text-muted">Reviews will appear here once prompts are purchased and rated.</p>
        )}
      </Reveal>
      ...
      <Reveal className="border-y border-indigo-500/20 bg-gradient-to-r from-indigo-900/30 to-cyan-900/30 px-4 py-20 text-center sm:px-6 lg:px-8">
        ...
        <Link to="/explore" className="mt-8 inline-flex">
          <Button size="lg" className="animate-glow">Explore Prompts</Button>
        </Link>
      </Reveal>
    </>
  );
